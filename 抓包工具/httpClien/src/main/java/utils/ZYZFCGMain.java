        ...
        public void run() {
            ...
            for (int i = 0; i < array.size(); i++) {
                try {
                   ...
                   // 拿到采购产品需求清单
                    String countData = StringUtils.substringBetween(contentStr, "采购产品需求清单", "</table>");
                  // 拿到td
                    String[] type=StringUtils.substringsBetween(countData,"<td style=\"width:99px;text-align: center\">","</td>");
                    //
                    String[] name=StringUtils.substringsBetween(countData,"<td style=\"width:220px;text-align: center\">","</td>");
                    for (int j = 0; j < type.length; j++) {
                        System.out.println("type:"+type[j]+"name:"+name[j]);

                        MyData myData = new MyData();
                        myData.setCount("无");
                        myData.setBudgeting("无");
                        myData.setPriceSuccess("wu");
                        myData.setUnit("wu ");
                        myData.setAddr("wu");
                        myData.setTime(type[j]);
                        myData.setName(name[j]);
...
//        String fileName = "数据3.xls";
      ...
      HSSFWorkbook workbook = exportExcel(myDataList, MyData.class);
        ...
        workbook.write(new File("D:/中央政府采购产品需求清单.xls"));
